...
async fn start_proxy(state: tauri::State<'_, AppState>) -> Result<(), String> {
    ...
    // Reject configurations the user clearly forgot to fill in. The proxy
    // would otherwise start, immediately fail to connect to "placeholder.onion",
    // and the GUI would show a confusingly successful "Running" status.
    let config = controller.get_config().await;
    if config.onion_endpoint.is_empty() || config.onion_endpoint == "placeholder.onion:80" {
...
fn main() {
    ...
    // The GUI is a trusted local client of the discovery API — opt it in by
    // default so the in-app wallet flows work, while leaving the CLI/server
    // user-controlled. The token is generated by the proxy at start time and
    // persisted to ${XDG_RUNTIME_DIR:-/tmp}/torpc-discovery.token (mode 0600);
    // the GUI reads that file and forwards it as `X-Torpc-Token` when
    // querying the discovery endpoint from any embedded wallet view.
