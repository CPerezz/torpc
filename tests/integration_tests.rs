...
async fn test_tor_hidden_service_accessibility() {
    ...
    match socks_test.get("http://check.torproject.org").send().await {
        ...
        Err(_) => {
            ...
            println!("  ⚠️  To test .onion access, you need a separate Tor instance with SOCKS enabled");
    ...
    let onion_url = format!("http://{}/rpc", onion_address);
    ...
    match client.post(&onion_url).json(&request_body).send().await {
        Ok(response) => {
            ...
            if status.is_success() {
                ...
                assert!(body.get("result").is_some(), "Response should have a result field");
            } else {
...
async fn test_concurrent_requests() {
    ...
    println!("  Sending {} concurrent requests...", num_concurrent_requests);
        ...
        let handle = tokio::spawn(async move {
            ...
            match make_rpc_request(TORPC_URL, method, None).await {
                Ok(response) => {
                    ...
                    assert!(response.get("result").is_some(), "Request {} failed: no result", i);
                    println!("    Request {} ({}) completed in {:?}", i, method, duration);
                    Ok(())
                }
                Err(e) => {
                    println!("    Request {} ({}) failed: {}", i, method, e);
                    Err(e)
                }
            }
        });
    ...
    println!("  Results: {} successful, {} failed", successes, failures);
    assert!(successes >= num_concurrent_requests * 90 / 100, "At least 90% of requests should succeed");
...
async fn test_malformed_json_handling() {
    ...
    for (description, malformed_json) in test_cases {
        ...
        if let Ok(body) = response.json::<Value>().await {
            println!("    Response: {}", serde_json::to_string_pretty(&body).unwrap_or_default());
...
async fn test_z_rate_limiting() {
    ...
    println!("  Sending {} requests to trigger rate limit...", num_requests);
    ...
    println!("  Results: {} successful, {} rate limited, {} other errors", 
             success_count, rate_limited_count, other_errors);
    
    // We should see some successes and some rate limits
    assert!(success_count > 0, "Should have some successful requests");
    assert!(success_count <= 100, "Should not exceed rate limit of 100 requests per minute");
...
async fn test_valid_rpc_methods() {
    ...
    let test_methods = vec![
        ...
        ("eth_getBalance", Some(json!(["0x0000000000000000000000000000000000000000", "latest"]))),
    ];
    ...
    for (method, params) in test_methods {
        ...
        while attempts < max_attempts {
            match make_rpc_request(TORPC_URL, method, params.clone()).await {
                Ok(response) => {
                    ...
                    println!("    ✓ {} responded correctly", method);
                    break;
                }
                Err(e) => {
                    attempts += 1;
                    ...
                    if attempts < max_attempts {
                        println!("    Retry {} for {}: {}", attempts, method, last_error);
        ...
        if attempts >= max_attempts {
            panic!("Method {} failed after {} attempts: {}", method, max_attempts, last_error);
        }
    }
        ...
        match response {
            Err(e) => {
                ...
                assert!(
                    ...
                    "Expected method not allowed error for {}, got: {}",
                    method,
                    error_str
                );
                println!("    ✓ {} correctly blocked", method);
            }
            Ok(_) => panic!("Method {} should have been blocked", method),
        }
    }
