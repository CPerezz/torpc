...
async fn wait_for_service(url: &str, timeout: Duration) -> Result<(), String> {
    ...
    loop {
        if start.elapsed() > timeout {
            return Err(format!("Service at {url} did not become ready in time"));
        }

        match client.get(url).send().await {
            Ok(resp) if resp.status().is_success() || resp.status().as_u16() == 404 => {
                println!("Service at {url} is ready");
                return Ok(());
            }
            _ => {
...
async fn test_mev_bundle_submission_without_key() {
    ...
    wait_for_service("http://127.0.0.1:8080", Duration::from_secs(10))
        .await
        .unwrap();
...
async fn test_mev_with_signing_key() {
    ...
    let mut torpc_process = Command::new("cargo")
        ...
        .env(
            ...
            std::env::var("TEST_FLASHBOTS_SIGNING_KEY").unwrap(),
        )
    ...
    wait_for_service("http://127.0.0.1:8081", Duration::from_secs(30))
        .await
        .unwrap();
    ...
    // wait() reaps the zombie. clippy::zombie_processes catches the missing
    // call; SIGKILL plus an immediate wait is the cleanest reaping pattern.
    let _ = torpc_process.wait();
...
async fn test_circuit_breaker_simulation() {
    ...
    wait_for_service("http://127.0.0.1:8082", Duration::from_secs(30))
        .await
        .unwrap();
